...
kue.redis.createClient = function() {
  var client = redis.createClient(6379, 'nodejitsudb3294128503.redis.irstack.com');
  client.auth('nodejitsudb3294128503.redis.irstack.com:f327cfe980c971946e80b8e975fbebb4');
  return client;
};
*/
...
// irc connect
var irchannel = '#components';

var irclient = new irc.Client('irc.freenode.com', 'component', {
    channels: [irchannel],
});
 ...
 *
 * - if doesn't exists into dB -> `add component`
 * - if exists into dB -> check versions -> `update component`
 * - checks if compoonent has a github page
 */
...
jobs.process('update component', 1, function(job, done){
  ...
  repo.info(function(err, gh){
    ...
    db.components.update(doc._id, { $set: {
      ...
      // check shortly github url
      component.shortly('https://github.com/' + doc.repo, function(err, short){
        if (err) return done(err);
        var say = twStatus('update: ', doc, (err ? '' : ' - ' + short), 400);
        irclient.say(irchannel, say);
        done();
      });
    });
  });
...
jobs.process('emit twitt', 1, function(job, done){
  ...
  component.shortly('https://github.com/' + doc.repo, function(err, short){
    ...
    var status = twStatus('new: ', doc, (err ? '' : ' - ' + short));
    tuitter.update({ status: status }, function(err, res){
      ...
      if (err) {
        log.error('twitting emision', { status: status, error: res });
        set.twitted = 'failed';
      } else {
        set.twitted = 'done';
      }
...
function twStatus(pre, doc, short, length){
  var pkg = JSON.parse(doc.pkg || doc.pck);
  var hashes = '';
